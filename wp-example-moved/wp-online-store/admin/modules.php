...
// ** GOOGLE CHECKOUT **
  ...
  if ($dir = @dir($module_directory)) {
    ...
    /*    if($_REQUEST['set']=="payment"){

        if (isset($HTTP_GET_VARS['list']) && ($HTTP_GET_VARS['list'] = 'new') && has_filter('the_content','wp_payment_filter') ) {
...
elseif($_REQUEST['set']=="shipping"){
	if(has_filter('the_content','wp_shipping_filter')){
	 if (isset($HTTP_GET_VARS['list'])) {
	    echo '            <td class="smallText" align="right">' . tep_draw_button(IMAGE_BACK, 'triangle-1-w', tep_href_link(FILENAME_MODULES, 'set=' . $set)) . '</td>';
	  } else {
	    echo '            <td class="smallText" align="right">' . tep_draw_button(IMAGE_MODULE_INSTALL . ' (' . $new_modules_counter . ')', 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&list=new')) . '</td>';
...
<?php
  ...
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
	...
	$mod = substr($file,0,strpos($file,'.')).'_code';
	if($$mod!='All right reserved by IMC group' && $HTTP_GET_VARS['set']=='payment')
...
?>
              <tr>
                ...
                <?php 
                if(($_REQUEST['set']=="payment" && !has_filter('the_content','wp_payment_filter')) || ($_REQUEST['set']=="shipping" && !has_filter('the_content','wp_shipping_filter')))
				{
					echo '            <tr><td class="smallText" align="right">' . TEXT_INSTALL_GOLD_STORE . '</td></tr>';
				}
           ?>    
    ...
    default:
      ...
      if ($mInfo->status == '1') {
        ...
        $keys = substr($keys, 0, strrpos($keys, '<br /><br />'));
         ...
         if($mInfo->code!='cod' && $mInfo->code!='paypal_standard' && $mInfo->code!='paypal_express' && $mInfo->code!='flat' && $mInfo->code!='table')
        	$remove_btn=tep_draw_button(IMAGE_MODULE_REMOVE, 'minus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove'));
        else
        	$remove_btn="";
        		
        $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_EDIT, 'document', tep_href_link(FILENAME_MODULES, 'set=' . $set . (isset($HTTP_GET_VARS['module']) ? '&module=' . $HTTP_GET_VARS['module'] : '') . '&action=edit')) .$remove_btn  );
        ...
        if($_GET['set']=="payment" && !has_filter('the_content','wp_payment_filter')){
        	if($_GET['module']=="paypal_standard" || $_GET['module']=="cod" || $_GET['module']=="paypal_express")	
          		...
          		$contents[] = array('align' => 'center', 'text' => TEXT_INSTALL_GOLD_STORE);	
        } 		
         elseif($_GET['set']=="shipping" && !has_filter('the_content','wp_shipping_filter')){
        	if($_GET['module']=="flat" || $_GET['module']=="table" )	
          		$contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_MODULE_INSTALL, 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install')));
          	else
          		$contents[] = array('align' => 'center', 'text' => TEXT_INSTALL_WP_SHIPPING);	
        } 	
        else  
		  $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_MODULE_INSTALL, 'plus', tep_href_link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install')));	
          if (isset($mInfo->signature) && (list($scode, $smodule, $sversion, $soscversion) = explode('|', $mInfo->signature))) {
