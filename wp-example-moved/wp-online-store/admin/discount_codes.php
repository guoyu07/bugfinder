...
function applies_to_onclick() {
  ...
  for (var i = 0, n = a.length; i < n; i++) if (a[i].checked) { b.disabled = (a[i].value == 2 || a[i].value == 4 ? false : true); c.disabled = (a[i].value == 3 || a[i].value == 5 ? true : false) }
}
...
function onload() {
  ...
  customers_onclick();
      ...
      <tr>
        <td><?php echo tep_draw_separator('pixel_trans.gif', '1', '10'); ?>To activate the discount code box on your store you must instal the module. Go to: MODULES / Order Total / Install Additional Modules / Discount Code</td>
      </tr>
      <?php echo tep_draw_form('new_discount_code', FILENAME_DISCOUNT_CODES, (isset($HTTP_GET_VARS['page']) ? 'page=' . $HTTP_GET_VARS['page'] . '&' : '') . 'action=insert' . (isset($HTTP_GET_VARS['dID']) ? '&dID=' . (int)$HTTP_GET_VARS['dID'] : '')); ?><tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            ...
            <td class="main" valign="top"><?php echo tep_draw_input_field('number_of_use', $dInfo->number_of_use, 'size="4"'); ?></td>
          </tr>
          <tr>
            ...
            <td class="main" valign="top"><?php echo tep_draw_input_field('number_of_products', $dInfo->number_of_products, 'size="4" id="number_of_products"'); ?></td>
          </tr>
          <tr>
            <td></td>
            <td></td>
            ...
            <td></td>
            <td></td>
            <td class="main"><?php echo '<label>' . tep_draw_checkbox_field('order_info', '1', $dInfo->order_info == 1) . '&nbsp;' . TEXT_ORDER_INFO . '</label>'; ?></td>
            <td></td>
          </tr>
        </table></td>
      </tr>
...
<?php
    ...
    $excluded_products_id = '<select name="excluded_products_id[]" size="10" multiple style="width: 280px;height:150px;" id="excluded_products_id">' . $products_id;
    $products_id = '<select name="products_id[]" size="10" multiple style="width: 280px;height:150px;">' . $products_id;
    ...
    $categories_id = str_replace('<select name="categories_id">', '<select name="categories_id[]" size="10" multiple style="width: 280px;height:150px;">', tep_draw_pull_down_menu('categories_id', tep_get_category_tree('0', '', '0')));
    ...
    $manufacturers_id = str_replace('<select name="manufacturers_id">', '<select name="manufacturers_id[]" size="10" multiple style="width: 280px;height:150px;">', tep_draw_pull_down_menu('manufacturers_id', $manufacturers_array));
...
?>
          ...
          <tr>
            ...
            <td></td>
<?php
    $customers_id = '<select name="customers_id[]" size="10" multiple style="width: 280px;height:150px;" id="customers_id">';
  ...
  switch ($action) {
    ...
    default:
      if (is_object($dInfo)) {
        ...
        if ($dInfo->order_info == 1) {
          $contents[] = array('text' => '' . tep_image(DIR_WS_ICONS . 'tick.gif', TABLE_HEADING_ORDER_INFO_FULL) . '&nbsp;' . TABLE_HEADING_ORDER_INFO_FULL);
        }
